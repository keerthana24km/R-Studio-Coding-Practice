...
student=function(){
  result=numeric(45)
  for(j in c(1:45)){
    result[j]=sample(s,size=1,replace=TRUE)
  }
  return(result)
}

nsimulations=1000
simulatedData=replicate(nsimulations,student())
str(simulatedData)
...
```{r}
s = c(1:365)
stud_strength = c(25,36,42,38,54)
student_matrix = matrix()
result = c()
for(i in (1:length(stud_strength))){
  student_matrix = matrix(numeric(stud_strength[i]), nrow = i, ncol = stud_strength[i])
}
print(student_matrix)
print(length(student_matrix[1, ]))


student=function(){
  for(sz in (1:length(student_matrix[ ,1]))){
    for(j in c(1:length(student_matrix[1, ]))){
      result[j]=sample(s,size=1,replace=TRUE)
      student_matrix[sz,j] = result[j]
    }
  }
...
mean(apply(simulatedData,1,checkEvent))*100
```

```{r}
s = c(1:365)
stud_strength = c(10:50)
resulting_probabilities = c(1:5)

for(sz in (1:length(stud_strength))){
  student=function(){
    result=numeric(stud_strength[sz])
    for(j in c(1:stud_strength[sz])){
      result[j]=sample(s,size=1,replace=TRUE)
    }
    return(result)
  }
  ...
  resulting_probabilities[sz] = mean(apply(simulatedData,2,checkEvent))*100
}
print(resulting_probabilities)
plot(resulting_probabilities)
```
